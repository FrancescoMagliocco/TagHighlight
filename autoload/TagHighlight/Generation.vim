...
if !exists('g:loaded_TagHighlight')
      \ || (exists('g:loaded_TagHLGeneration')
...
function! s:UpdateTypesFile()
  ...
  " Debug information for configuration
  if TagHighlight#Debug#DebugLevelIncludes('Information')
        \ && option_file_info['Exists']
    call TagHLDebug(
          \ 'Project config file options: '
          \   . string(b:TagHighlightConfigFileOptions),
          \ 'Information')
  else
  ...
  if ctags_option ==? 'None'
    ...
    call TagHLDebug("CtagsExecutable not set, searching for 'ctags' in path",
          \ 'Information')
    let b:TagHighlightSettings['CtagsExeFull'] =
          \ TagHighlight#RunPythonScript#FindExeInPath('ctags')
  elseif ctags_option =~? '[\\/]'
    " Option set and includes '/' or '\': must be explicit
    " path to named executable: just pass to mktypes
    call TagHLDebug(
          \ 'CtagsExecutable set with path delimiter, using as explicit path',
          \ 'Information')
    ...
    " in the path
    call TagHLDebug(
          \ 'CtagsExecutable set without path delimiter, searching in path',
          \ 'Information')
    let b:TagHighlightSettings['CtagsExeFull'] =
          \ TagHighlight#RunPythonScript#FindExeInPath(ctags_option)
  endif
  ...
  if tag_file_info['Found'] == 1
    let b:TagHighlightSettings['CtagsFileLocation'] =
          \ tag_file_info['Directory']
  endif

  let types_file_info = TagHighlight#Find#LocateFile('TYPES', '*')
  if types_file_info['Found'] == 1
    let b:TagHighlightSettings['TypesFileLocation'] =
          \ types_file_info['Directory']
  endif
  ...
  if TagHighlight#Option#GetOption('SourceDir') =~? 'None'
    ...
    if ! TagHighlight#Option#GetOption('Recurse')
      call TagHLDebug(
            \ 'Non-recursive mode, using file directory', 'Information')
      ...
      call TagHLDebug(
            \ 'File is ' . file . '(' . fnamemodify(file, ':p:h') . ')',
            \ 'Information')
    ...
    call TagHLDebug(
          \ 'Source dir set explicitly to '
          \ . TagHighlight#Option#GetOption('SourceDir'), 'Information')
  endif
  ...
  if TagHighlight#Option#GetOption('OnlyGenerateTypesIfPresent') == 1
    if !types_file_info['Exists']
      call TagHLDebug(
            \ 'Types file does not exist, not generating new files',
            \ 'Information')
      return
    endif
  endif
  ...
  if tag_file_info['Exists'] == 1
    if TagHighlight#Option#GetOption('DoNotGenerateTagsIfPresent') == 1
      ...
      call TagHLDebug(
            \ "Tag file doesn't exist and DoNotGenerateTagsIfPresent set, not "
            \ . 'generating new tag file', 'Information')
      let b:TagHighlightSettings['DoNotGenerateTags'] = 1
    endif
  elseif TagHighlight#Option#GetOption('DoNotGenerateTags') == 1
    echoerr 'Cannot create types file without generating tags: tags file does'
          \ 'not exist'
    return
  endif
  ...
  if has('cscope')
    if TagHighlight#Option#GetOption('EnableCscope')
      ...
      if b:TagHighlightPrivate['CscopeFileInfo']['Found'] == 1
        let b:TagHighlightSettings['CscopeFileLocation'] =
              \ b:TagHighlightPrivate['CscopeFileInfo']['Directory']
      endif
    endif
  endif
  ...
  for postupdate_hook in postupdate_hooks
    call TagHLDebug(
          \ 'Calling post-update hook ' . postupdate_hook, 'Information')
    exe 'call' postupdate_hook . '()'
  endfor

  call TagHLDebug(
        \ 'UpdateTypesFile() complete, current directory is now ' . getcwd(),
        \ 'Information')
endfunction

function! TagHighlight#Generation#UpdateAndRead(skiptags)
  call TagHLDebug(
        \ 'UpdateAndRead() called with parameter ' . a:skiptags, 'Information')
