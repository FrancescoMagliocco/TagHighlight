...
if !exists('g:loaded_TagHighlight')
      \ || (exists('g:loaded_TagHLFind') && g:plugin_development_mode != 1)
  finish
endif
...
function! TagHighlight#Find#LocateFile(which, suffix, ...)
  call TagHLDebug(
        \ 'Locating file ' . a:which . ' with suffix ' . a:suffix,
        \ 'Information')
  ...
  let default_priority = TagHighlight#Option#GetOption(
        \ 'DefaultDirModePriority', force_project)
  call TagHLDebug('Priority: ' . string(default_priority), 'Information')
  let default_search_wildcards = TagHighlight#Option#GetOption(
        \ 'DefaultDirModeSearchWildcards', force_project)

  if len(force_project)
    let file = TagHighlight#Option#GetOption('SourceDir', force_project)
          \ . '/dummy.txt'
  else
    " Can we get away with just using 'expand('%')'?
  ...
  if a:which ==? 'TAGS'
    ...
    let search_priority = TagHighlight#Option#GetOption(
          \ 'TagFileDirModePriority', force_project)
    let explicit_location = TagHighlight#Option#GetOption(
          \ 'TagFileDirectory', force_project)
    let search_wildcards = TagHighlight#Option#GetOption(
          \ 'TagFileSearchWildcards', force_project)
  elseif a:which ==? 'TYPES'
    let search_suffix = a:suffix ==? 'all' ? '*' : a:suffix
    let filename =
          \ TagHighlight#Option#GetOption('TypesFilePrefix', force_project)
          \ . '_' . search_suffix . '.' . TagHighlight#Option#GetOption(
          \   'TypesFileExtension', force_project)
    let search_priority = TagHighlight#Option#GetOption(
          \ 'TypesFileDirModePriority', force_project)
    let explicit_location = TagHighlight#Option#GetOption(
          \ 'TypesFileDirectory', force_project)
    let search_wildcards = TagHighlight#Option#GetOption(
          \ 'TypesFileSearchWildcards', force_project)
  elseif a:which == 'CONFIG'
    " Suffix is ignored here
    let filename = TagHighlight#Option#GetOption(
          \ 'ProjectConfigFileName', force_project)
    let search_priority = TagHighlight#Option#GetOption(
          \ 'ProjectConfigFileDirModePriority', force_project)
    let explicit_location = TagHighlight#Option#GetOption(
          \ 'ProjectConfigFileDirectory', force_project)
    let search_wildcards = TagHighlight#Option#GetOption(
          \ 'ProjectConfigFileSearchWildcards', force_project)
  elseif a:which ==? 'AUTOSOURCE'
    " Suffix is ignored here
    let filename =
          \ TagHighlight#Option#GetOption('AutoSourceFileName', force_project)
    let search_priority = TagHighlight#Option#GetOption(
          \ 'AutoSourceFileDirModePriority', force_project)
    let explicit_location = TagHighlight#Option#GetOption(
          \ 'AutoSourceFileDirectory', force_project)
    let search_wildcards = TagHighlight#Option#GetOption(
          \ 'AutoSourceFileSearchWildcards', force_project)
  elseif a:which ==? 'CSCOPE'
    " Suffix is ignored here
    let filename =
          \ TagHighlight#Option#GetOption('CscopeFileName', force_project)
    let search_priority = TagHighlight#Option#GetOption(
          \ 'CscopeFileDirModePriority', force_project)
    let explicit_location = TagHighlight#Option#GetOption(
          \ 'CscopeFileDirectory', force_project)
    let search_wildcards = TagHighlight#Option#GetOption(
          \ 'CscopeFileSearchWildcards', force_project)
  else
  ...
  if TagHighlight#Option#GetOption('UseProjectRepository', force_project)
    ...
    let repository =
          \ TagHighlight#Option#GetOption('ProjectRepository', force_project)
    if len(repository) > 0 && tolower(repository) !=? 'none'
      if len(force_project) > 0
            \ || (has_key(b:TagHighlightPrivate, 'InProject')
            \ && b:TagHighlightPrivate['InProject'])
        let project_name = len(force_project)
              \ ? force_project
              \ : b:TagHighlightPrivate['ProjectName']
        let project_folder = repository . '/' . s:SanitiseName(project_name)
  ...
  for search_mode in search_priority
    ...
    elseif search_mode ==? 'CurrentDirectory'
      call TagHLDebug(
            \ 'Using current directory: ' . fnamemodify('.', ':p:h'),
            \ 'Information')
      let result['Directory']   = fnamemodify('.',':p:h')
      let result['Filename']    = filename
    elseif search_mode ==? 'FileDirectory'
      call TagHLDebug(
            \ 'Using file directory: ' . fnamemodify(file, ':p:h'),
            \ 'Information')
    ...
    if has_key(result, 'Directory')
      ...
      let result['Found']       = 1
      call TagHLDebug(
            \ 'Found file location: ' . result['FullPath'], 'Information')
...
function! s:ScanUp(dir, wildcards)
  ...
  call TagHLDebug(
        \ 'Searching up from ' . a:dir . ' for ' . string(a:wildcards),
        \  'Information')
  ...
  while new_dir != dir
    ...
    for wildcard in a:wildcards
      ...
      if len(glob_result)
        ...
        if found
          ...
          call TagHLDebug(
                \ 'Wildcard matches were not readable (directory?)',
                \ 'Information')
        endif
      endif
    endfor
    ...
    let levels += 1
    if max_levels && levels >= max_levels
